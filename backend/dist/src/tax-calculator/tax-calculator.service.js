...
const MANAGEMENT_FEE = 11.00;
const DELIVERY_FEE = 2.76;
let TaxCalculatorService = class TaxCalculatorService {
    ...
    async calculateTax(dto) {
        ...
        if (!regionalRate) {
            throw new common_1.NotFoundException(`Département ${departmentCode} non trouvé`);
        }
        const isCleanVehicle = [tax_calculator_dto_1.FuelType.ELECTRIQUE, tax_calculator_dto_1.FuelType.HYDROGENE, tax_calculator_dto_1.FuelType.HYBRIDE].includes(fuelType);
        ...
        if (exonerationApplied) {
            if (fuelType === tax_calculator_dto_1.FuelType.ELECTRIQUE || fuelType === tax_calculator_dto_1.FuelType.HYDROGENE) {
                exonerationRate = 100;
                regionalTax = 0;
            }
            else if (fuelType === tax_calculator_dto_1.FuelType.HYBRIDE) {
                exonerationRate = 50;
        ...
        if (isNew && co2Emissions && transactionType === 'ACHAT_NEUF') {
            const malusThreshold = ECO_MALUS_2024.find((t) => co2Emissions >= t.min && (t.max === null || co2Emissions <= t.max));
